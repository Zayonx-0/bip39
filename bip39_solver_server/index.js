...
var db = new sqlite3.Database("bip39.sqlite", function(){
  db.run("CREATE TABLE IF NOT EXISTS batch_counter (start INTEGER)", function(){
    db.get("SELECT * FROM batch_counter", function(err, row){
      if(row === undefined) {
        db.run("INSERT INTO batch_counter (start) VALUES (0)");
      }
    });
  });
  db.run("CREATE TABLE IF NOT EXISTS batches (id INTEGER PRIMARY KEY, start INTEGER NOT NULL UNIQUE, batch_size INTEGER NOT NULL, status INTEGER NOT NULL, created_at TEXT DEFAULT CURRENT_TIMESTAMP)", function(){
    db.run("CREATE INDEX IF NOT EXISTS batches_status_idx ON batches (status)");
  });
});
...
let knownWords = ["vacuum", "certain", "enrich", "finger", "answer", "solid", "kind", "smooth", "radio", "burst", "wrestle", "liar", "yard", "allow", "ritual", "caught", "easily", "pulp", "swarm", "now"];
let batchSize = 100000; // 16777216
let solution = null;
let indices = knownWords.map(knownWord => words.findIndex(word => word === knownWord));

app.get('/work', (req,res) => {
  console.log("requested work")
  if(!req.query.secret || req.query.secret !== SHARED_SECRET){
    return res.json({});
  }

  lock.acquire('safe-work', function(cb){
    db.get("SELECT * FROM batch_counter", function(err, counter){
      if(counter) {
        db.run("INSERT INTO batches (start, batch_size, status) VALUES (?,?,?)", [counter.start, batchSize, 0], function(){
          let nextStart = counter.start + batchSize;
          db.run("UPDATE batch_counter SET start=?",[nextStart], function(){
            cb(null, counter.start);
          })
        })
      }
    })
  }, function(err, start){
    res.json({ indices: indices, batch_size: batchSize, offset: start});
  });
});

app.post("/work", (req,res) => {
  if(!req.body.secret || req.body.secret !== SHARED_SECRET){
    return res.json({});
  }
  db.get("SELECT * FROM batches WHERE start=?", [req.body.offset], function(err, batch){
    if(!batch){
      return res.json({error: 'invalid offset'});
    }

    if(batch.status === 1) {
      return res.json({error: 'already completed'});
    } else {
      db.run("UPDATE batches SET status=1 WHERE start=?", [req.body.offset], function(){
        res.json({});
      });
    }
  });
...
app.post("/mnemonic", (req,res) => {
  ...
  solution = req.body.mnemonic.replace(/\0/g, '');
  res.json({});
});

app.post("/updatebatchsize", (req,res) => {
  if(!req.body.secret || req.body.secret !== SHARED_SECRET){
    return res.json({});
  }
  if(!isNaN(parseInt(req.body.batch_size, 10))){
    batchSize = parseInt(req.body.batch_size, 10);
  }
  res.json({});
});

app.post("/updatemnemonic", (req,res) => {
  if(!req.body.secret || req.body.secret !== SHARED_SECRET){
    return res.json({});
  }
  const newWords = req.body.mnemonic.split(" ");
  const invalidWord = newWords.find((word) => {
    return words.findIndex(w => w === word) === -1
  });

  if(invalidWord){
    res.json({error: 'looks like you have a typo'});
  } else if(knownWords.join(" ") === req.body.mnemonic) {
    res.json({error: 'looks like these are the same as current'});
  } else {
    db.run("UPDATE batch_counter SET start=0", function(err){
      db.run("DELETE FROM batches", function(err){
        db.run("VACUUM", function(err){
          knownWords = newWords;
          indices = knownWords.map(knownWord => words.findIndex(word => word === knownWord));
          res.json({});
        })
      })
    })
...
app.get('/status', (req,res) => {
  ...
  db.get("SELECT * FROM batch_counter", function(err, counter){
    db.get("SELECT sum(batch_size) as completed FROM batches WHERE status=1 and created_at >= datetime('now','-1 hour')", function(err,work){
      const total = 2**(128 - (knownWords.length * 11)); 
      const pct = ((counter.start / total) * 100.0).toFixed(2);
      const hoursLeft = (total / work.completed).toFixed(2);
      res.json({words: knownWords, total: total, current: counter.start, hours_left: hoursLeft, percent_complete: pct, completed_last_hour: work.completed, batch_size: batchSize, solution: solution});
    })
  });
});

app.listen(port, () => console.log(`bip39 server listening at http://localhost:${port}`));